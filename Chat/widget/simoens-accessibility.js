(function () {
  var IS_EMBEDDED_FRAME = window.self !== window.top;
  ...
  function preferenceSnapshot() {
    return {
      font: Math.max(0, Math.min(3, Number(state.font) || 0)),
      ...
      colorblindType: state.colorblindType === 'blueyellow' ? 'blueyellow' : 'redgreen'
    };
  }

  function applyPreferencePayload(payload) {
    if (!payload || typeof payload !== 'object') return false;
    var next = preferenceSnapshot();
    Object.keys(next).forEach(function (key) {
      if (!Object.prototype.hasOwnProperty.call(payload, key)) return;
      if (key === 'font') next.font = Math.max(0, Math.min(3, Number(payload.font) || 0));
      else if (key === 'colorblindType') next.colorblindType = payload.colorblindType === 'blueyellow' ? 'blueyellow' : 'redgreen';
      else next[key] = payload[key] === true;
    });
  ...
  function sameOriginFrame(frame) {
    ...
    try {
      var source = frame.getAttribute('src') || 'about:blank';
      if (source === 'about:blank' || source.indexOf('javascript:') === 0) return true;
  ...
  function sendPreferencesToFrame(frame) {
    if (!sameOriginFrame(frame)) return;
    try {
      frame.contentWindow.postMessage(
        { type: 'simoens-a11y-preferences', payload: preferenceSnapshot() },
        location.origin === 'null' ? '*' : location.origin
      );
    } catch (error) {}
  }

  function propagatePreferencesToFrames() {
    if (IS_EMBEDDED_FRAME) return;
    document.querySelectorAll('iframe').forEach(function (frame) {
      sendPreferencesToFrame(frame);
      if (frame.getAttribute('data-simoens-a11y-bridge') === 'true') return;
      frame.setAttribute('data-simoens-a11y-bridge', 'true');
      frame.addEventListener('load', function () { sendPreferencesToFrame(frame); });
    });
  }

  function ensurePreferenceLayers() {
    if (!document.querySelector('.simoens-a11y-guide-line')) {
      var guide = document.createElement('div');
      guide.className = 'simoens-a11y-guide-line';
      guide.setAttribute('aria-hidden', 'true');
      document.body.appendChild(guide);
      document.addEventListener('mousemove', function (event) {
        if (state.guide) guide.style.top = event.clientY + 'px';
      }, { passive: true });
    }
    if (!document.querySelector('.simoens-a11y-grayscale-layer')) {
    ...
    if (!document.querySelector('.simoens-a11y-colorblind-layer')) {
  ...
  function installFramePreferenceBridge() {
    if (!IS_EMBEDDED_FRAME) return;
    window.addEventListener('message', function (event) {
      var trustedOrigin = event.origin === location.origin || (location.origin === 'null' && event.origin === 'null');
      if (event.source !== parent || !trustedOrigin) return;
      if (!event.data || event.data.type !== 'simoens-a11y-preferences') return;
      if (!applyPreferencePayload(event.data.payload)) return;
      saveState();
      applyPreferences();
    });
    try {
      parent.postMessage({ type: 'simoens-a11y-frame-ready' }, location.origin === 'null' ? '*' : location.origin);
    } catch (error) {}
  }

  if (!IS_EMBEDDED_FRAME) {
    window.addEventListener('message', function (event) {
      if (!event.data || event.data.type !== 'simoens-a11y-frame-ready') return;
      var trustedOrigin = event.origin === location.origin || (location.origin === 'null' && event.origin === 'null');
      if (!trustedOrigin) return;
      var frame = Array.prototype.find.call(document.querySelectorAll('iframe'), function (item) { return item.contentWindow === event.source; });
      if (frame) sendPreferencesToFrame(frame);
    });
  }
...
.simoens-a11y-trigger:hover {
  ...
  function start() {
    ...
    loadState();
    if (IS_EMBEDDED_FRAME) {
      ensurePreferenceLayers();
      installFramePreferenceBridge();
      applyPreferences();
      applyFixes();
      loadAnimationAccessibilityExtension();
      var frameObserver = new MutationObserver(function () {
        window.clearTimeout(window.__simoensA11yFrameTimer);
        window.__simoensA11yFrameTimer = window.setTimeout(function () {
          applyFixes();
          applyPreferences();
        }, 120);
      });
