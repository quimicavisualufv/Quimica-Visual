(function () {
  ...
  function loadScript() {
    ...
    script.onerror = function () {
      ...
      root.dataset.simoensVlibrasStatus = 'unavailable';
      const button = root.querySelector('[vw-access-button]');
      if (button) {
        button.setAttribute('aria-disabled', 'true');
        button.setAttribute('aria-label', 'VLibras indisponível sem conexão com o serviço externo');
        button.setAttribute('title', 'VLibras indisponível no momento');
      }
      window.dispatchEvent(new CustomEvent('simoens:vlibras-error'));
    };
