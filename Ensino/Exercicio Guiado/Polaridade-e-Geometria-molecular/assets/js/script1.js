...
(function(){
  ...
  function resize(){
    ...
    if(angleCanvas){
      ...
      if(angleCtx) angleCtx.setTransform(dpr,0,0,dpr,0,0);
    }
  ...
  function geomPositions(type){
    ...
    switch(type){
      ...
      case 'trigonal_pyramidal': return {lig:[tetra[1],tetra[2],tetra[3]], lp:[tetra[0]], angle:'~107° (<109,5°; 1 par livre)', ideal:'~107°', arr:'AX3E', label:'Piramidal trigonal (AX3E)'};
      case 'bent_tet': {
        var a=30*DEG, b=52.25*DEG;
        return {lig:[[Math.sin(b),0,-Math.cos(b)],[-Math.sin(b),0,-Math.cos(b)]], lp:[[Math.sin(a),0,Math.cos(a)],[-Math.sin(a),0,Math.cos(a)]], angle:'~104,5° (<109,5°; 2 pares livres)', ideal:'~104,5°', arr:'AX2E2', label:'Angular (AX2E2)'};
      }
  ...
  var currentGeomData = geomPositions(geom ? geom.value : 'tetrahedral');
  ...
  var __practiceElementPalette = {
    ...
    Pd:'#d8dde5', Au:'#f2cf63', Ni:'#86b2a1', Ti:'#95aacc', Zr:'#99b0d0', Mo:'#a8b2c7', W:'#aab3c3', U:'#79d07a',
    As:'#9ea9bf', Co:'#6b7fb6', Cr:'#8aa2c6', CN:'#8ce2df', NH3:'#d7c0ff'
  };
  function __practiceColorForLabel(lbl, fallback){
    var key = String(lbl||'').replace(/[^A-Za-z0-9]/g,'');
    return __practiceElementPalette[key] || fallback;
  }
  ...
  function moleculeWorld(){
    ...
    var practiceRender = window.__geomPracticeRenderData || null;
    var centerBase = colorize(coreColor.value, parseFloat(sat.value)||1);
    var centerClr = practiceRender && practiceRender.centerColor ? colorize(practiceRender.centerColor, parseFloat(sat.value)||1) : centerBase;
    var baseCore = rotXYZ([0,0,0], rot);
    var centerPos = vAdd(baseCore, __sceneMolOffset);
    var atoms = [{ pos:centerPos, atom:{ color:centerClr, scale:90 * clamp(coreR/0.42, 0.8, 2.6), label:(practiceRender && practiceRender.center) ? practiceRender.center : '' }, kind:'center', dir:[0,0,0], radius:coreR }];
    var bonds = [];
    var lps = [];
    G.lig.forEach(function(dir, idx){
      var pos = vAdd(rotXYZ(vMul(dir, bl), rot), __sceneMolOffset);
      var ligLabel = (practiceRender && practiceRender.ligands && practiceRender.ligands[idx]) ? practiceRender.ligands[idx] : '';
      var ligBase = colorize(ligandColor.value, parseFloat(sat.value)||1);
      var ligHex = (practiceRender && practiceRender.ligandColors && practiceRender.ligandColors[idx]) ? practiceRender.ligandColors[idx] : __practiceColorForLabel(ligLabel, ligandColor.value);
      atoms.push({ pos:pos, atom:{ color:colorize(ligHex, parseFloat(sat.value)||1), scale:86 * clamp(ligR/0.34, 0.8, 2.3), label:ligLabel }, kind:'ligand', dir:rotXYZ(dir, rot), radius:ligR });
  ...
  function applyAutoFraming(world){
    ...
    var spanX = Math.max(0.4, maxAbsRX * 2);
    var spanY = Math.max(0.4, maxAbsRY * 2);
    var frameWidth = (canvas.clientWidth||1) * (__sceneSplit ? 0.82 : 0.62);
    var frameHeight = (canvas.clientHeight||1) * 0.62;

    state.center = anchor;
    state.fitScale = Math.min(frameWidth/spanX, frameHeight/spanY) * 0.84;
  ...
  function __viewerPadPx(){
    try{
      var raw = getComputedStyle(document.documentElement).getPropertyValue('--viewer-pad') || '0';
  ...
  function drawBondWorld(bd){
    ...
    var spacing = Math.max(3.4, (parseFloat(bondRadius.value)||0.085) * 38);
    var lw = Math.max(1.4, (parseFloat(bondRadius.value)||0.085) * 24 * 0.92);
  ...
  function __practiceAngleHidden(){
    try {
      return !!(window.__practiceIsActive && window.__practiceIsActive() && window.__practiceState && !window.__practiceState.revealed);
    } catch(err){
      return false;
    }
  }
  ...
  function buildAngleEntries(world){
    ...
    return pairs.map(function(pair, idx){
      ...
      var label = formatAngleText(value);
      var hidden = __practiceAngleHidden();
      ...
      return {
        ...
        value: value,
        label: label,
        displayLabel: hidden ? 'X °' : label,
        overlayLabel: hidden ? '?' : label,
  ...
  function getAngleLegendLabel(entry, entries){
    ...
    var base = entry.displayLabel || entry.label;
    return repeated > 1 ? base + ' · ' + entry.duplicateIndex : base;
  }
  ...
  function updateAngleLegend(entries){
    ...
    var masterOn = !!(showAngles && showAngles.checked);
    var masterSig = '|m=' + (masterOn ? 1 : 0);
    if(!entries.length){
      if(__angleLegendSignature !== 'empty' + masterSig){
        angleLegendPanel.innerHTML = '';
        var emptyHead = document.createElement('div');
        emptyHead.className = 'angleLegendHead';
        var emptyTitle = document.createElement('div');
        emptyTitle.className = 'angleLegendTitle';
        emptyTitle.textContent = 'Ângulos';
        emptyHead.appendChild(emptyTitle);
        angleLegendPanel.appendChild(emptyHead);
        var emptyText = document.createElement('div');
        emptyText.className = 'angleLegendEmpty';
        emptyText.textContent = masterOn ? 'Sem ângulos visíveis nessa geometria.' : 'Ative “Mostrar ângulos” no menu à esquerda.';
        angleLegendPanel.appendChild(emptyText);
        __angleLegendSignature = 'empty' + masterSig;
      }
      return;
    }
    var sig = entries.map(function(entry){ return [entry.id, entry.label, isAngleEntryVisible(entry) ? 1 : 0, entry.color].join(':'); }).join('|') + masterSig;
    if(sig === __angleLegendSignature) return;
    __angleLegendSignature = sig;
    angleLegendPanel.innerHTML = '';
    var head = document.createElement('div');
    head.className = 'angleLegendHead';
    var title = document.createElement('div');
    title.className = 'angleLegendTitle';
    title.textContent = 'Ângulos';
    head.appendChild(title);
    angleLegendPanel.appendChild(head);
    ...
    entries.forEach(function(entry){
      ...
      checkbox.id = 'angleLegendToggle_' + entry.id;
      checkbox.name = 'angleLegendToggle_' + entry.id;
      ...
      meta.textContent = masterOn ? 'Ocultar este ângulo' : 'Ative “Mostrar ângulos”';
  ...
  function drawAnglesOverlay(world){
    ...
    try {
      practiceRunning = !!(window.__practiceIsActive && window.__practiceIsActive());
    } catch(err){
    ...
    __currentAngleEntries.forEach(function(entry, idx){
      ...
      drawAngleChip(centerP.x + Math.cos(mid) * labelRadius, centerP.y + Math.sin(mid) * labelRadius, entry.overlayLabel || entry.displayLabel || entry.label, accent);
    });
  ...
  function drawLonePair(entry){
    var base = 10.6 * clamp(entry.radius / 0.24, 0.78, 1.72);
    var coreR = parseFloat(size.value) || 0.64;
    var centerW = __sceneMolOffset;
    var dirWorld = entry && entry.dir ? entry.dir.slice() : vSub((entry && entry.pos) || centerW, centerW);
    var dirLen = Math.hypot(dirWorld[0] || 0, dirWorld[1] || 0, dirWorld[2] || 0) || 1;
    dirWorld = [(dirWorld[0] || 0) / dirLen, (dirWorld[1] || 0) / dirLen, (dirWorld[2] || 0) / dirLen];
    var lpWorldDist = Math.max(0.06, coreR * 0.22 + entry.radius * 0.06);
    var cloudWorld = vAdd(centerW, vMul(dirWorld, lpWorldDist));
    var p = project(cloudWorld);
  ...
  function drawBalloonWorld(entry){
    ...
    var base = 18 * clamp((sx + sy + sz) / 3, 0.72, 2.15);
  ...
  function autoDist(){
    var bl = parseFloat(bondLen.value) || 0.625;
    var R = Math.max(parseFloat(size.value)||0.64, parseFloat(ligandRadius.value)||0.44) + bl + 0.9;
  ...
  function sync(){
    ...
    var __practiceHideAngle = __practiceAngleHidden();
    var __idealHudText = __practiceHideAngle ? '?' : currentGeomData.ideal;
    infoIdeal.textContent = __idealHudText;
    document.getElementById('hud').innerHTML = '<b>' + currentGeomData.label + '</b> · <span class="small">' + currentGeomData.arr + ' - ' + __idealHudText + '</span>';
  ...
  window.__geomTutorialAPI = {
    ...
    projectWorld: function(world){ return project(world); },
    rotateCameraVec: function(v){ return rotateCamera(v); },
    getProjectionMeta: function(){ return { center: state.center ? state.center.slice() : [0,0,0], fitScale: state.fitScale, screenOffset: state.screenOffset ? state.screenOffset.slice() : [0,0], zoom: getEffectiveZoomFactor() }; },
