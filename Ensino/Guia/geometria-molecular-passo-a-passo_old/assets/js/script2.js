...
(function(){
  ...
  function balloonDemoHTML(){
    ...
    var cols='';
    for(var i=1;i<=n;i++){
      cols += '<div class="balloonCol balloonPalette-'+(((i-1)%6)+1)+'" title="Balão '+i+'">'
           +    '<div class="balloonTop"></div><div class="balloonKnot"></div><div class="balloonString"></div></div>';
    }
    var rows='';
    ...
    return ''
      ...
      +     '</div>'
      +   '</div>'
      +   '<div class="balloonStage">'+cols+'<div class="balloonGround"></div></div>'
      +   '<div class="balloonLegend"><b>Ideia:</b> imagine vários balões presos no mesmo nó. Quanto mais balões você coloca, mais eles precisam se reorganizar para caber sem se espremer. Na molécula, esse papel é das <b>regiões eletrônicas</b>.</div>'
  ...
  async function applyBalloonDemoToBoard(count, animate){
    ...
    if(el.examples && isBalloonStep(steps[stepIdx])) el.examples.innerHTML = balloonDemoHTML();
  ...
  function getWorldDataForExample(ex){
    ...
    var lps = (G.lp||[]).map(function(d){
      ...
      return {dir:dir, pos:vMul(dir, bl*1.05)};
    });
...
function projectPoint(world, view, size){
  ...
  function __getOverlayBondParams(){
    var ui = apiRef && apiRef.ui;
    return {
      bondLen: (ui && ui.bondLen) ? (parseFloat(ui.bondLen.value)||1.25) : 1.25,
  ...
  function __bondRailCandidates(centerWorld, ligWorld, dir, bondType){
    ...
    var L = vLen(vSub(ligWorld, centerWorld)) || params.bondLen || 1.25;
...
function drawDensitySurfaceMap(ctx, data, view, can){
    ...
    (data.bondDipoles||[]).forEach(function(d){
      ...
      var pos = vAdd(d.anchor||[0,0,0], vMul(negDir, (data.bondLen||1.55)*0.24));
    ...
    (data.lps||[]).forEach(function(lp){
      ...
      var ang = Math.atan2(p2.y-p.y,p2.x-p.x);
      drawCloudBlob(ctx, p.x, p.y, Math.max(30, envBase.pxU*0.72), Math.max(19, envBase.pxU*0.45), isLightBG?[182,190,200]:[120,205,255], isLightBG?0.08:0.105, ang);
    });
