...
(function(){
  ...
  function resize(){
    ...
    var nextCW = Math.round(w*dpr);
    var nextCH = Math.round(h*dpr);
    var sizeChanged = (canvas.width !== nextCW || canvas.height !== nextCH);
    if(sizeChanged){
      canvas.width = nextCW;
      canvas.height = nextCH;
      __needsAutoFrame = true;
    }
    if(angleCanvas){
      var angleChanged = (angleCanvas.width !== nextCW || angleCanvas.height !== nextCH);
      if(angleChanged){
        angleCanvas.width = nextCW;
        angleCanvas.height = nextCH;
        __needsAutoFrame = true;
      }
  ...
  function localToWorldPoint(v){
    return vAdd(rotXYZ(v, rot), __sceneMolOffset);
  }

  function localToWorldDir(v){
    return rotXYZ(v, rot);
  }

  function projectLocal(v){
    return project(localToWorldPoint(v));
  }
  ...
  function moleculeWorld(){
    ...
    var atoms = [{ pos:centerPos, atom:{ color:colorize(coreColor.value, parseFloat(sat.value)||1), scale:90 * clamp(coreR/0.42, 0.8, 2.6), label:'' }, kind:'center', dir:[0,0,0], radius:coreR }];
    var bonds = [];
    var lps = [];
  ...
  function draw(){
    ...
    if(__needsAutoFrame || !isFinite(state.fitScale) || state.fitScale <= 0){
      applyAutoFraming(world);
      __needsAutoFrame = false;
    }
    drawCoordinateAxes();
    var items=[];
  ...
  window.__geomTutorialAPI = {
    ...
    projectLocal: function(local){ return projectLocal(local); },
    localToWorld: function(local){ return localToWorldPoint(local); },
    localDirToWorld: function(local){ return localToWorldDir(local); },
