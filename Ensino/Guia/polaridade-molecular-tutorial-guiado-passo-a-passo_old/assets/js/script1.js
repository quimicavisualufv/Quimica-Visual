...
(function(){
  ...
  function __setLightBG(v){
    ...
    if(typeof prog!=='undefined' && prog && typeof U!=='undefined' && U){
      ...
      if(U.uBGStars){
        var stars = v ? 0.35 : parseFloat(bgStars && bgStars.value || 1.0);
        gl.uniform1f(U.uBGStars, stars);
      }
      if(U.uBGBright){
        var bright = v ? 1.0 : parseFloat(bgBrightness && bgBrightness.value || 1.0);
        gl.uniform1f(U.uBGBright, bright);
      }
    }
  }
...
save.addEventListener('click', function(){ requestAnimationFrame(function(){ var a=document.createElement('a'); a.download='geometria_molecular_3d.png'; a.href=canvas.toDataURL('image/png'); a.click(); }); });
  ...
  function __syncAnglesFromEye(){
    ...
    rotY = __closestAngle(Math.atan2(__eyeVec[0], __eyeVec[2]), rotY);
  }
  ...
  canvas.addEventListener('pointermove', function(e){
    ...
    rotY = rotY + dx * orbitSpeed;
    rotX = __clamp(rotX + dy * orbitSpeed, -Math.PI/2 + 0.05, Math.PI/2 - 0.05);
    __syncOrbitFromAngles();
  });
  canvas.addEventListener('wheel', function(e){ e.preventDefault(); camDist*=(1+Math.sign(e.deltaY)*0.08); camDist=Math.max(2.2, Math.min(14, camDist)); __syncOrbitFromAngles(); }, {passive:false});
  canvas.addEventListener('dblclick', function(){ camDist=autoDist(); rotX=0.35; rotY=-0.6; __syncOrbitFromAngles(); rot.x=rot.y=rot.z=0; rotXS.value=rotYS.value='0'; if(rotZS) rotZS.value='0'; if(tutorialRotZVal) tutorialRotZVal.textContent='0°'; });
...
float probDots(vec3 p, float seed){
      ...
      for(int i=0;i<96;i++){
        ...
        if(h.d<0.0015){
          ...
          if(h.lp>1.5){
            ...
            float specB = pow(max(dot(reflect(-ldir,n), -rd), 0.0), 90.0);
            float fresB = pow(1.0-max(dot(n,-rd),0.0), 3.2);
            vec3 envSoft = envMap(reflect(-rd,n))*0.10;
            vec3 latex = h.col*(0.50 + 0.72*diff*sh + 0.18*diff2) + vec3(rim)*0.07;
            latex = saturateColor(latex, 1.18);
            col = clamp(latex + envSoft + vec3(specB)*0.42 + vec3(fresB)*0.04, 0.0, 1.0);
          }
  ...
  function __setBalloons3D(n, dirs){
    ...
    var centralR = parseFloat(size.value)||1.0;
    var radXY = centralR * 0.50;
    var radZ = radXY * 0.68;
    var dist = centralR + radZ * 0.98;
    ...
    for(var i=0;i<6;i++){
      ...
      if(i<n && dirs && dirs[i]){
        ...
        __balloons.rad[bi+0]=radXY;
        __balloons.rad[bi+1]=radXY*0.92;
        __balloons.rad[bi+2]=radZ;
  ...
  function miniResize(){
    ...
    var rw = Math.max(1, Math.round(w*dpr));
    var rh = Math.max(1, Math.round(h*dpr));
    if(miniCanvas.width !== rw || miniCanvas.height !== rh){
      miniCanvas.width = rw;
      miniCanvas.height = rh;
  ...
  function __closestAngle(target, reference){
    if(!isFinite(target)) return isFinite(reference) ? reference : 0;
    if(!isFinite(reference)) return target;
    var two = Math.PI * 2;
    return reference + __wrapAngle(target - reference);
  }
  ...
  function __animateView(opts,dur){
    ...
    var e = {
      ...
      rotY: ('rotY' in opts)? __closestAngle(opts.rotY, s.rotY) : s.rotY,
  ...
  window.__geomTutorialAPI = {
    ...
    setView: function(o){ o=o||{}; if('camDist' in o) camDist=__clamp(o.camDist,2.2,14); if('rotX' in o) rotX=__clamp(o.rotX,-Math.PI/2+.05,Math.PI/2-.05); if('rotY' in o) rotY=__closestAngle(o.rotY, rotY); __syncOrbitFromAngles(); if(('objRotXDeg' in o)||('objRotYDeg' in o)||('objRotZDeg' in o)) __setObjRotDeg(o.objRotXDeg||0,o.objRotYDeg||0,o.objRotZDeg||0); },
