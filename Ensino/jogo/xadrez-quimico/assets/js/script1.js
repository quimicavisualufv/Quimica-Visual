...
function calculateBest(chess, moves, depth) {
    ...
    const material = useMemo(() => {
        ...
        return new MeshPhysicalMaterial({
            transmission: 0,
            opacity: isAmberGlass ? 0.42 : 0.32,
            transparent: true,
            metalness: 0,
            roughness: 0.08,
            ior: 1.45,
            thickness: 0,
            color: isAmberGlass ? '#c96a1a' : (isWhite ? '#f8fafc' : '#d9e2ec'),
            clearcoat: 1,
            clearcoatRoughness: 0.05,
            emissive: isHighlighted ? '#aaffaa' : (isAmberGlass ? '#2b1305' : '#000000'),
            emissiveIntensity: isHighlighted ? 0.4 : (isAmberGlass ? 0.025 : 0),
            side: DoubleSide,
            depthWrite: false,
        });
    ...
    const liquidMat = useMemo(() => {
        ...
        let opacity = 0.85;
        if (isAmber) {
            // Keep reagent-bottle liquids clearly differentiated by team color, even through amber glass.
            color = isWhite ? '#22c55e' : '#2563eb';
            emissive = isWhite ? '#064e3b' : '#0c4a6e';
            opacity = 0.98;
        }
...
function Rook({ material, liquid, liquidRef, themeId }) {
    ...
    if (themeId === 3) {
        ...
        // Corrige a lacuna do vidro conectando o corpo com o topo arredondado.
        return (_jsxs("group", { position: [0, 0.5, 0], children: [_jsx("mesh", { material: material, position: [0, -0.4, 0], children: _jsx("cylinderGeometry", { args: [0.3, 0.3, 0.05, 32] }) }), _jsx("mesh", { material: material, position: [0, -0.04, 0], children: _jsx("cylinderGeometry", { args: [0.3, 0.3, 0.78, 32, 1, false] }) }), _jsx("mesh", { material: material, position: [0, 0.34, 0], children: _jsx("cylinderGeometry", { args: [0.298, 0.298, 0.06, 32, 1, false] }) }), _jsx("mesh", { material: material, position: [0, 0.35, 0], children: _jsx("sphereGeometry", { args: [0.3, 32, 16, 0, Math.PI * 2, 0, Math.PI / 2] }) }), _jsx("mesh", { material: material, position: [0, 0.5, 0], children: _jsx("cylinderGeometry", { args: [0.15, 0.15, 0.3, 32, 1, false] }) }), _jsx("mesh", { position: [0, 0.65, 0], material: new MeshStandardMaterial({ color: '#222222' }), children: _jsx("cylinderGeometry", { args: [0.16, 0.16, 0.08, 16] }) }), _jsx("group", { ref: liquidRef, children: _jsx("mesh", { material: liquid, position: [0, -0.15, 0], children: _jsx("cylinderGeometry", { args: [0.28, 0.28, 0.5, 32] }) }) })] }));
    }
    if (themeId === 4) {
...
function App() {
    ...
    const checkGameOver = () => {
        if (chess.isCheckmate()) {
            setGameOver(`Xeque-mate! ${chess.turn() === 'w' ? 'Azuis' : 'Verdes'} vencem.`);
            playVictorySound();
        }
